...
export default function Dashboard() {
  ...
  const handleBookingAction = async (bookingId: string, action: "accepted" | "rejected") => {
    try {
      await apiRequest("PATCH", `/api/bookings/${bookingId}`, { status: action });
  ...
  // ✅ HELPER: Get status string (pending, accepted, etc.)
                      ...
                      return (
                        <div key={booking._id || booking.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-3 bg-white border rounded-lg shadow-sm">
                          ...
                          <div className="flex gap-2 w-full sm:w-auto">
                            <Button size="sm" className="flex-1 sm:flex-none bg-green-600 hover:bg-green-700 h-8" onClick={() => handleBookingAction(booking._id || booking.id, 'accepted')}>
                              Accept
                            </Button>
                            <Button size="sm" variant="outline" className="flex-1 sm:flex-none text-red-600 border-red-200 hover:bg-red-50 h-8" onClick={() => handleBookingAction(booking._id || booking.id, 'rejected')}>
                              Decline
                            </Button>
                          </div>
                        </div>
                      ...
                      {acceptedPassengers.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {acceptedPassengers.map((booking: any) => (
                            <div key={booking._id || booking.id} className="flex items-center gap-3 p-2 border rounded-md bg-card">
                              <Avatar className="h-8 w-8">
                                <AvatarImage src={booking.passengerId?.avatar} />
                                <AvatarFallback>{booking.passengerId?.name?.[0] || "P"}</AvatarFallback>
                              </Avatar>
                              <div className="flex-1">
                                <p className="text-sm font-medium">{booking.passengerId?.name || "Passenger"}</p>
                                <p className="text-xs text-muted-foreground">{booking.passengerId?.phone || "No Phone"}</p>
                              </div>
                              <Badge variant="secondary">{booking.seatsBooked} Seat(s)</Badge>
                            </div>
                          ))}
                        </div>
                      )}
                ...
                <Card>
                  ...
                  <CardContent>
                    {filteredRides && filteredRides.filter(r => r.driverId === user?.id && r.status === 'scheduled').length > 0 ? (
                      ...
                      <EmptyState icon={Car} title="No scheduled rides" description="Post a ride to get started." action={{ label: "Post a Ride", onClick: () => setLocation("/post-ride") }} />
                    )}
                  </CardContent>
                </Card>
              )}
      ...
      <div className="flex flex-col lg:flex-row h-[calc(100vh-4rem)]">
        <div className="lg:w-2/5 xl:w-1/3 p-4 lg:p-6 border-r overflow-hidden flex flex-col">
          ...
          <div className="relative mb-4">
            ...
            <Input placeholder="Search by location or driver..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
          </div>
          ...
          <ScrollArea className="flex-1 -mr-4 pr-4">
            {ridesLoading ? (
              ...
              <EmptyState icon={MapPin} title="No rides available" description="Check back later or try a different search." />
            )}
          </ScrollArea>
        </div>
        ...
        <DialogContent className="max-w-4xl p-0 h-[500px] overflow-hidden">
          {trackingRide && (
            <div className="h-full w-full">
              <MapComponent
                // Center on driver's current position if available, else source
                center={[
